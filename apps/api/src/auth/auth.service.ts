...
export class AuthService {
  ...
  async registerWithSupabase(dto: SignupDto) {
    ...
    if (error) {
      ...
      // If duplicate, try to sign in to get the user and session
      ...
      if (signInError || !signInData.user) {
        throw new ConflictException(
          'User already registered in authentication system. Please login.'
        );
      }
      ...
      // New user created, now sign in to get session
      const { data: signInData, error: signInError } = await this.supabaseService
        .getClient()
        .auth.signInWithPassword({
          email: dto.email,
    ...
    try {
      ...
      const fallbackSupabaseUser = supabaseUser || {
      ...
      return {
        ...
        session: {
          ...
          expires_in: 900,
          token_type: 'bearer',
          user: fallbackSupabaseUser as any,
        },
      };
    } catch (dbError) {
  ...
  async signupBusiness(dto: SignupBusinessDto) {
    ...
    if (authError) {
      ...
      if (!signInError && signInData.session) {
        authData = { user: data.user, session: signInData.session };
      } else {
  ...
  async login(dto: LoginDto) {
    ...
    // Sign in with Supabase to get tokens
  ...
  async logout(userId: string) {
    try {
      // Sign out from Supabase (invalidates all sessions for admin signout)
      await this.supabaseService.getClient().auth.admin.signOut(userId, 'global');
    } catch (error) {
      // Log but don't fail - local logout still valid
   ...
   */
  async deleteAccount(userId: string, confirmEmail: string, reason?: string) {
    // Get user
    const user = await this.prisma.user.findUnique({
      where: { id: userId },
      include: {
    ...
    // Verify email matches (double confirmation)
    if (user.email.toLowerCase() !== confirmEmail.toLowerCase()) {
      throw new BadRequestException('Email confirmation does not match');
    }

    // Check for active businesses - require them to be transferred/deleted first
    if (user.businesses.length > 0) {
      throw new BadRequestException({
        message: 'Cannot delete account with active businesses',
        businesses: user.businesses.map(b => ({ id: b.id, name: b.name })),
        action: 'Please transfer or delete your businesses before deleting your account',
      });
    }
    ...
    if (user.providerId) {
      try {
        await this.supabaseService.getClient().auth.admin.deleteUser(user.providerId);
      } catch (error) {
    ...
    // This will cascade to related records based on schema relations
    await this.prisma.$transaction(async (tx) => {
      ...
      // Delete user's messages (senderId in Message model)
      await tx.message.deleteMany({ where: { senderId: userId } });
      
      // Delete user's chats (as participant)
