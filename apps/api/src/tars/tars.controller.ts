...
import { TarsContext, TarsService } from './tars.service';
...
class ChatDto {
  ...
  @IsOptional()
  @IsIn(['general', 'help', 'business', 'booking', 'analytics', 'messages', 'schedule'])
  ...
  @IsOptional()
  @IsIn(['guest', 'customer', 'business'])
  persona?: 'guest' | 'customer' | 'business';
...
export class TarsController {
  ...
  async chat(@Body() body: ChatDto, @Req() req: AuthenticatedRequest) {
    ...
    // Determine persona based on auth state and request
    let persona: 'guest' | 'customer' | 'business' = body.persona || 'guest';
    if (req.user) {
      // If user is authenticated, upgrade from guest
      if (req.user.role === 'BUSINESS_OWNER' || body.context === 'business') {
        persona = 'business';
    ...
    return {
      ...response,
      sessionId,
      persona, // Return the determined persona
    };
  }
  ...
  async getConversation(
    ...
    const messages = await this.tarsService.getConversationHistory(conversationId, limit);
    return { messages };
  }
  ...
  @UseGuards(JwtAuthGuard, AdminGuard)
  @HttpCode(HttpStatus.OK)
  async bulkReviewActions(@Body() body: ReviewActionDto, @Req() req: AuthenticatedRequest) {
    return this.actionsService.bulkReview(body.actionIds, body.decision, req.user!.id, body.notes);
  }
  ...
  @UseGuards(JwtAuthGuard)
  async updateSettings(@Param('businessId') businessId: string, @Body() body: UpdateSettingsDto) {
