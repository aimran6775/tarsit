...
export class UploadsController {
  ...
  @ApiBody({
    schema: {
      ...
      properties: {
        publicId: {
          type: 'string',
          description: 'Storage path (same value returned as publicId from uploads)',
          example: 'tarsit/businesses/1700000000000-photo.jpg',
        },
        path: {
          type: 'string',
  ...
  deleteImage(@Body('publicId') publicId?: string, @Body('path') path?: string) {
    const storagePath = publicId ?? path;
    if (!storagePath) {
      throw new BadRequestException('publicId (or path) is required');
    }
    return this.uploadsService.deleteImage(storagePath);
  }
}
