 ...
 *   node scripts/test-backend-ready.js --url=http://localhost:4001
 *
 * Environment Variables:
 *   API_URL - The base URL of the API (default: http://localhost:4001)
  ...
  if (signup.success && signup.data.user) {
    ...
    if (login.success && login.data.accessToken) {
      ...
      const errorMsg = signup.error
        ? JSON.stringify(signup.error).substring(0, 100)
        : 'Unknown error';
