...
import { Bot, Loader2, Minimize2, RefreshCw, Send, Sparkles, X } from 'lucide-react';
...
export function TarsChat({
    ...
    async (messageText: string) => {
      ...
      try {
        ...
        const fallbackMessage: Message = {
          ...
          role: 'assistant',
          content:
            "I'm having a bit of trouble connecting right now. But don't worry - I'm still here! Try asking me something simple, or we can connect you with our support team.",
          timestamp: new Date(),
  ...
  const chatContent = (
    <>
      ...
      <form onSubmit={handleSubmit} className="p-4 border-t border-white/10">
        ...
        <p className="text-xs text-white/30 mt-2 text-center">"{randomQuote}"</p>
      </form>
    </>
  );
  ...
  if (embedded) {
    return (
      <div
        className={`flex flex-col h-full bg-neutral-900/50 rounded-2xl border border-white/10 overflow-hidden ${className}`}
      >
        {chatContent}
      </div>
    );
  ...
  return (
    <div
      className={`fixed bottom-6 right-6 w-96 h-[32rem] bg-neutral-900 rounded-2xl border border-white/10 shadow-2xl shadow-black/50 flex flex-col overflow-hidden z-50 ${className}`}
    >
      {chatContent}
    </div>
  );
