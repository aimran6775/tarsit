...
const DEFAULT_PUBLIC_PAGE_SECTION_ORDER: PublicPageSectionKey[] = [
  'about',
  'photos',
  'services',
  'reviews',
...
function Dashboard() {
  ...
  const [pageSettings, setPageSettings] = useState<PageSettings>({
    publicPagePrimaryCta: 'book',
    publicPageSectionOrder: DEFAULT_PUBLIC_PAGE_SECTION_ORDER,
  });
  ...
  // Allow deep-linking to a specific tab via ?tab=profile (etc)
  useEffect(() => {
      ...
      try {
        ...
        if (businessData?.id) {
          ...
          setVisibilitySettings({
            showAbout: businessData.showAbout ?? true,
            showAddress: businessData.showAddress ?? true,
            ...
            showMap: businessData.showMap ?? true,
            showPhotos: businessData.showPhotos ?? true,
          ...
          const rawOrder = businessData.publicPageSectionOrder;
          const sanitizedOrder = Array.isArray(rawOrder)
            ? rawOrder.filter((k: unknown): k is PublicPageSectionKey =>
                ['about', 'photos', 'services', 'reviews', 'location'].includes(String(k))
              )
            : [];

          setPageSettings({
            publicPagePrimaryCta:
              businessData.publicPagePrimaryCta === 'message' ? 'message' : 'book',
            publicPageSectionOrder:
              sanitizedOrder.length > 0 ? sanitizedOrder : DEFAULT_PUBLIC_PAGE_SECTION_ORDER,
          });
        }
      } catch (error) {
  ...
  const handleSaveSettings = async () => {
    ...
    try {
      ...
      await apiClient.patch(`/businesses/${business.id}`, {
        showAbout: visibilitySettings.showAbout,
        showAddress: visibilitySettings.showAddress,
        ...
        showMap: visibilitySettings.showMap,
        showPhotos: visibilitySettings.showPhotos,
        ...
        publicPagePrimaryCta: pageSettings.publicPagePrimaryCta,
        publicPageSectionOrder: pageSettings.publicPageSectionOrder,
      });
