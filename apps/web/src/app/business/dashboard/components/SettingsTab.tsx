...
import type { AppointmentSettings, PageSettings, PublicPageSectionKey } from '../types';
...
const SECTION_LABELS: Record<PublicPageSectionKey, string> = {
  about: 'About',
  photos: 'Photos',
  services: 'Services',
  reviews: 'Reviews',
...
const ALL_SECTIONS: PublicPageSectionKey[] = ['about', 'photos', 'services', 'reviews', 'location'];
...
function ToggleSwitch({
  ...
  return (
    ...
    >
      <div
        className={`${s.thumb} bg-white rounded-full shadow-sm transition-transform ${
...
function SettingsSection({
  ...
  return (
    <div
      className={`bg-white/5 backdrop-blur-xl rounded-2xl border ${borderColor} overflow-hidden`}
    >
      <button
      ...
      >
        <div className="flex items-center gap-4">
          <div
            className={`w-10 h-10 rounded-xl ${variant === 'danger' ? 'bg-red-500/20' : 'bg-gradient-to-br from-purple-500/20 to-indigo-500/20'} flex items-center justify-center`}
          >
            <Icon
              className={`w-5 h-5 ${variant === 'danger' ? 'text-red-400' : 'text-purple-400'}`}
            />
          </div>
...
export function SettingsTab({
  ...
  const effectiveSectionOrder: PublicPageSectionKey[] = (() => {
    const unique = Array.from(new Set(pageSettings.publicPageSectionOrder));
    const kept = unique.filter((k): k is PublicPageSectionKey => ALL_SECTIONS.includes(k));
    const missing = ALL_SECTIONS.filter((k) => !kept.includes(k));
    return [...kept, ...missing];
  })();

  const moveSection = (key: PublicPageSectionKey, direction: 'up' | 'down') => {
    const current = effectiveSectionOrder;
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <div className="space-y-3">
          <ToggleRow
            ...
            description="Display your description on the public page"
            enabled={visibility.showAbout}
            onChange={() => updateVisibility('showAbout')}
            iconColor="text-purple-400"
          />
          <ToggleRow
            icon={ImageIcon}
            title="Show Photos"
            description="Display your photo gallery on the public page"
            enabled={visibility.showPhotos}
            onChange={() => updateVisibility('showPhotos')}
          ...
          <ToggleRow
            icon={MapPin}
            title="Show Address"
            description="Display your address in the contact section"
            enabled={visibility.showAddress}
            onChange={() => updateVisibility('showAddress')}
          ...
          <ToggleRow
            ...
            description="Display the map on your public page"
            enabled={visibility.showMap}
            onChange={() => updateVisibility('showMap')}
      ...
      >
        ...
        {appointmentSettings.appointmentsEnabled && (
          <div className="mt-6 pt-6 border-t border-white/10">
            <h4 className="text-sm font-medium text-white/70 mb-4">Appointment Configuration</h4>
            <div className="grid sm:grid-cols-3 gap-4">
              <div>
                ...
                >
                  <option value={15} className="bg-neutral-900">
                    15 minutes
                  </option>
                  ...
                  <option value={120} className="bg-neutral-900">
                    120 minutes
                  </option>
                </select>
              </div>
              ...
              <div>
                <label className="block text-sm font-medium text-white/70 mb-2">Buffer Time</label>
                <select
              ...
              <div>
                ...
                >
                  ...
                  <option value={14} className="bg-neutral-900">
                    2 weeks
                  </option>
                  <option value={30} className="bg-neutral-900">
                    1 month
      ...
      <SettingsSection
        title="Page Layout"
        description="Set your section order and primary call-to-action"
        icon={FileText}
        defaultExpanded={false}
      >
        <div className="space-y-6">
          <div>
            <h4 className="text-sm font-medium text-white/70 mb-2">Primary CTA</h4>
            <select
              value={pageSettings.publicPagePrimaryCta}
              onChange={(e) =>
                setPageSettings({
                  ...pageSettings,
                  publicPagePrimaryCta: e.target.value === 'message' ? 'message' : 'book',
                })
              }
              className="w-full px-3 py-2.5 bg-white/5 border border-white/10 rounded-xl text-white text-sm focus:outline-none focus:ring-2 focus:ring-purple-500/50"
            >
              <option value="book" className="bg-neutral-900">
                Book (primary)
              </option>
              <option value="message" className="bg-neutral-900">
                Message (primary)
              </option>
            </select>
            <p className="text-xs text-white/40 mt-2">
              This controls which button is emphasized on your public page.
            </p>
          </div>

          <div>
            <h4 className="text-sm font-medium text-white/70 mb-2">Section Order</h4>
              ...
              {effectiveSectionOrder.map((key, idx) => (
                ...
                >
                  <div className="flex items-center gap-3 min-w-0">
                    <div className="w-7 h-7 rounded-lg bg-white/10 border border-white/10 flex items-center justify-center text-xs text-white/70 flex-shrink-0">
                      {idx + 1}
                    </div>
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-white/40">Shown on your public page</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <button
                      ...
                      disabled={idx === 0}
                      className="px-3 py-1.5 text-xs text-white/70 border border-white/10 rounded-lg hover:bg-white/10 disabled:opacity-40 disabled:hover:bg-transparent transition-colors"
                    >
                      Up
                    </button>
                    <button
                      ...
                      disabled={idx === effectiveSectionOrder.length - 1}
                      className="px-3 py-1.5 text-xs text-white/70 border border-white/10 rounded-lg hover:bg-white/10 disabled:opacity-40 disabled:hover:bg-transparent transition-colors"
                    >
                      Down
                    </button>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-white/50 mt-1">
                  Permanently delete your business listing and all associated data. This action
                  cannot be undone.
                </p>
          ...
          <div className="p-4 bg-amber-500/10 border border-amber-500/20 rounded-xl">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-white/50 mt-1">
                  Hide your business from search results and public view. You can unhide it anytime.
                </p>
