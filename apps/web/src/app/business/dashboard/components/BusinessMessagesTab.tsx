...
export function BusinessMessagesTab({ businessId, currentUserId }: BusinessMessagesTabProps) {
  ...
  useEffect(() => {
    const fetchChats = async () => {
      try {
        const res = await apiClient.get('/chats/business');
        const data = res.data as unknown;
        const chatData: Chat[] = Array.isArray(data)
          ? (data as Chat[])
          : (((data as { chats?: Chat[] })?.chats || []) as Chat[]);

        setChats(chatData.filter((c) => c.business?.id === businessId));
      } catch (error) {
  ...
  useEffect(() => {
    ...
    const fetchMessages = async () => {
      ...
      try {
        const res = await apiClient.get(`/messages/${selectedChat.id}`);
        const messagesData = (res.data as any)?.messages || res.data || [];
  ...
  const filteredChats = chats.filter((chat) => {
    ...
    const name = `${chat.user.firstName} ${chat.user.lastName}`.toLowerCase();
  ...
  return (
    <div className="bg-white/5 rounded-xl border border-white/10 overflow-hidden">
      <div className="flex h-[600px]">
        {/* Chat List */}
        <div
          className={`w-full sm:w-80 border-r border-white/10 flex flex-col ${selectedChat ? 'hidden sm:flex' : 'flex'}`}
        >
          {/* Header */}
            ...
            {filteredChats.length === 0 ? (
              ...
              filteredChats.map((chat) => (
                <button
                  ...
                  className={`w-full p-4 flex items-start gap-3 hover:bg-white/5 transition text-left ${
