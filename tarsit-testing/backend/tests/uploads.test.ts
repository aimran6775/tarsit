...
async function testUploads(context) {
  ...
  results.push(await runTest('POST /uploads/image - Upload single image', async () => {
    ...
    // May fail if Cloudinary not configured, which is acceptable
    if (response.status === 500 || response.status === 503) {
      throw new Error('Upload service not configured (Cloudinary)');
    }
  ...
  results.push(await runTest('POST /uploads/images - Upload multiple images', async () => {
    if (!context.tokens.customerToken) {
      throw new Error('No auth token available');
    }
