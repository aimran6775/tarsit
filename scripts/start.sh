...
# Kill processes on port 4001 (backend API)
if lsof -i :4001 > /dev/null 2>&1; then
    echo "  → Killing process on port 4001 (backend)..."
...
if lsof -i :4001 > /dev/null 2>&1; then
    echo -e "${RED}  ✗ Port 4001 is still in use!${NC}"
    exit 1
fi

echo -e "${GREEN}  ✓ Ports 3001 and 4001 are available${NC}"
echo ""
...
echo -e "${YELLOW}[4/5] Starting Backend API server (port 4001)...${NC}"
...
ATTEMPTS=0
MAX_ATTEMPTS=60
while ! curl -s http://localhost:4001/api/health > /dev/null 2>&1; do
    sleep 2
    ATTEMPTS=$((ATTEMPTS + 1))
...
echo -e "  ${GREEN}Backend:${NC}  http://localhost:4001/api"
echo ""
